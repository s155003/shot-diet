...
jumpers looks like a bad one.

Shot Diet pulls **1,087,633 regular-season field goal attempts** across five seasons
(2021-22 through 2025-26), splits every player's efficiency into those two components,
...
In a single season the two look equally decisive, varying across players by almost
exactly the same amount. Strip out measurement noise and **59% of the genuinely
repeatable difference between players is simply which shots they take.**

They also pull against each other (**r = −0.27**): players handed the easiest shots are
the weaker shot-makers, and the best shot-makers are handed the hardest shots. That is
why raw efficiency flatters the first group and buries the second.
...
The project ships a prescriptive optimiser: a linear program that reallocates 5% of a
player's attempts across six zones to maximise expected points, using empirical-Bayes
estimates of his zone-by-zone shooting. Graded against the **following** season, it is
worth **+2.49 points per 100 shots** and helps 99.5% of players.
...
**Personalising the shot diet to the individual shooter is worth nothing.** All of the
gain comes from the league-average zone structure: get out of the mid-range. This is
not an artefact of a tight constraint, because the null holds from a 3% to a 20% move
...
- **Shot quality belongs to the scheme far more than to the roster.** Selection shows
  up in eleven attempts and persists at r = 0.90 across seasons. It is the
  high-leverage, low-variance lever.
...
fit: `xPPS-loc` on geometry alone, and `xPPS-full` adding play type. The second drives
the headline split, on the reasoning that whether a shot is a cut, a pull-up or a
...
- **The API silently truncates.** `shotchartdetail` caps every response at 102,400
  ...
  error and no warning. The loader pages by calendar month and **asserts** no chunk
...
- **Clock management removed.** 3,696 backcourt attempts and buzzer heaves are
  dropped, since they are not shot selection.
- **Zones kept coarse on purpose.** An earlier ten-zone scheme split left from right;
  ...
  other *for the same shot*. That is binomial noise rather than a talent difference,
  and it made the optimiser prefer left-side threes to identical right-side ones. Six
...
- **The prescriptive tool is graded out of sample**, against two controls, with
  bootstrap CIs. That is how we found the null above rather than shipping a
  personalisation feature that does nothing.

### What this cannot see

- **No defender.** The public feed has no defender distance, so "selection" means
  location, play type and clock, but not whether the shot was open. Some of what lands
  in *making* is really the ability to create separation.
...
- **The optimiser assumes zone rates hold under reallocation.** They will not exactly,
  since the marginal corner three is not the average one. The small default move budget
  is there to keep that assumption honest.
...
The dashboard opens at `http://localhost:8501`. Every processed table is committed, so
**nothing is downloaded and no model is fitted at startup.** The app never contacts
stats.nba.com at runtime.
...
`fetch.py` writes to `data/raw/` (gitignored, ~30 MB) and skips seasons it already has.
`run_pipeline.py` overwrites `data/processed/` and `reports/summary.json`.
...
src/features.py       shot events into a modelling table, zones, exclusions
